...
2. Add property → **URL prefix** → `https://thulanimaseko.co.za/`.
...
7. Submit sitemap: **Sitemaps** → add `https://thulanimaseko.co.za/sitemap.xml`.
...
3. **Import sites from Google Search Console** → select `thulanimaseko.co.za`.
4. (Optional) Add `VITE_BING_VERIFICATION` token to `.env` + Workers secret if importing fails and manual verification is needed.
5. Submit sitemap: **Sitemaps** → `https://thulanimaseko.co.za/sitemap.xml`.
...
- [ ] At least one URL indexed (`site:thulanimaseko.co.za` query returns ≥ 1 result on both google.com and bing.com).
