...
class CodeExecutionService {
  ...
  async execute(data: CodeExecutionData): Promise<ApiResponse<CodeExecution>> {
    return httpClient.post<ApiResponse<CodeExecution>>(`${this.basePath}/execute`, data, {
      skipAuth: true,
  ...
  async getSupportedLanguages(): Promise<ApiResponse<string[]>> {
    return httpClient.get<ApiResponse<string[]>>(`${this.basePath}/languages`, { skipAuth: true });
  }
}
